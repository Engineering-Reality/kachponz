...
class MCPAutoManager:
    ...
    def _parse_mcp_tools(self, tools_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for tool in tools_data:
            ...
            command = f"mcp-proxy --port {port} {env_flags} -- {args}".strip()
            print(command)
    ...
    def _start_single_tool(self, tool: Dict[str, Any]) -> Optional[subprocess.Popen]:
        ...
        try:
            ...
            # --- OVERRIDE NODE.JS PATH ---
            # Inject NVM Node v20 path to fix ERR_INTERNAL_ASSERTION in Node 18
            nvm_path = os.path.expanduser("~/.nvm/versions/node/v20.20.2/bin")
            if os.path.exists(nvm_path):
                env["PATH"] = f"{nvm_path}{os.pathsep}{env.get('PATH', '')}"
                print(f"✅ Injected Node 20 into PATH for tool {tool_name}")
