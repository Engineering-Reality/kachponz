...
export default function InteractiveAuthGradient({ title, description }: { title: string; description: string }) {
  ...
  return (
    ...
    >
      ...
      {/* Aurora Thread — mesh variant, ambient ombient glow */}
      <AuroraThread variant="mesh" className="z-0" />

      {/* Interactive cursor gradient — same aurora spectrum, follows pointer */}
      <div
        className="absolute inset-0 transition-opacity duration-700 ease-out z-0 mix-blend-screen"
        style={{
          opacity: isHovering ? 0.8 : 0.4,
          background: `radial-gradient(circle at ${mousePosition.x}% ${mousePosition.y}%, rgba(217,70,239,0.6), rgba(59,130,246,0.6) 30%, rgba(34,211,238,0.2) 60%, transparent 80%)`,
          filter: 'blur(50px)'
        }}
      />
      ...
      <div className="relative z-20 flex flex-col items-center text-center px-12 pointer-events-none">
         ...
         <h2 className="text-3xl font-semibold text-white mb-4 tracking-tight">{title}</h2>
         <p className="text-white/60 text-sm max-w-xs leading-relaxed font-medium">
           {description}
         </p>
      </div>

      {/* Floating abstract elements — aurora spectrum */}
      <div className="absolute top-10 left-10 w-32 h-32 bg-[var(--aurora-blue)] rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-pulse z-0" />
      <div className="absolute bottom-10 right-10 w-40 h-40 bg-[var(--aurora-fuchsia)] rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-pulse z-0" style={{ animationDelay: '2s' }} />
    </div>
  );
}
