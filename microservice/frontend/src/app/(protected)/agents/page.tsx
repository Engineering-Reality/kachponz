...
export default function AgentsPage() {
  ...
  const toggleStar = (agentId: string) => {
    setStarredAgents((prev) => {
      ...
      localStorage.setItem("starred-agents", JSON.stringify(next));
      return next;
    });
  };
  const [toolsList, setToolsList] = useState<any[]>([]);
  const [knowledgeBasesList, setKnowledgeBasesList] = useState<any[]>([]);
  ...
  const fetchAgentsAndTools = async () => {
    ...
    try {
      ...
      setKnowledgeBasesList(kbRes.ok ? (await kbRes.json() || []) : []);
    } catch (err: any) {
  ...
  useEffect(() => { 
    fetchAgentsAndTools(); 
    const stored = localStorage.getItem("starred-agents");
    if (stored) setStarredAgents(JSON.parse(stored));
  }, []);

  const openCreateModal = () => {
    setModalMode("create");
    setCurrentAgent(null);
    setFormData({ agent_name: "", description: "", agent_style: "The agent will reply in a warm and friendly manner, using English.", on_status: true, tools: [], knowledge_base_ids: [] });
  ...
  const openEditModal = async (agent: any) => {
    ...
    setFormData({ agent_name: agent.agent_name || "", description: agent.description || "", agent_style: agent.agent_style || "", on_status: agent.on_status ?? true, tools: agent.tools || [], knowledge_base_ids: agent.knowledge_base_ids || [] });
    setRecipeError(null);
    setIsModalOpen(true);
  ...
  const filteredAgents = agents.filter(agent => {
    ...
    const aStarred = starredAgents.includes(a.agent_id);
    const bStarred = starredAgents.includes(b.agent_id);
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      ...
      <div className="page-header border-b border-slate-100 dark:border-slate-800 pb-6 mb-6">
        <div>
          <p className="ui-label text-slate-400 dark:text-slate-500 mb-2">Agent Registry</p>
          <h1 className="section-head text-3xl text-slate-900 dark:text-white mb-1">Agent Gallery</h1>
        ...
        <div className="flex items-center gap-3">
          <button onClick={fetchAgentsAndTools} className="btn-secondary text-xs py-2.5 px-4 rounded-xl shadow-sm bg-white dark:bg-slate-900 hover:bg-slate-50 dark:bg-slate-800/50 transition-all">
        ...
        <div className="flex flex-col md:flex-row gap-4 justify-between items-center mb-8 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-4 rounded-2xl shadow-sm">
          <div className="relative w-full md:w-80">
            <input
              ...
              className="w-full pl-9 pr-4 py-2 border border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-fuchsia-500/20 focus:border-fuchsia-500"
            />
            <Bot className="w-4 h-4 text-slate-400 dark:text-slate-500 absolute left-3 top-3" />
          </div>
          <div className="flex items-center gap-2 w-full md:w-auto overflow-x-auto pb-1 md:pb-0">
            <span className="text-xs font-mono text-slate-400 dark:text-slate-500 mr-2 flex-shrink-0">Filter Status:</span>
            {[
              ...
              <button
                ...
                className={`text-xs font-semibold px-3 py-1.5 rounded-lg border transition-all ${statusFilter === tab.id ? "bg-fuchsia-50 dark:bg-fuchsia-900/30 border-fuchsia-200 dark:border-fuchsia-500/30 text-fuchsia-700 dark:text-fuchsia-400 shadow-sm" : "bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300 hover:bg-slate-50 dark:bg-slate-800/50" }`}
              >
                {tab.label}
              </button>
            ))}
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-24">
          <RainbowRibbonLoader />
          <p className="mt-4 text-xs font-mono text-slate-400 dark:text-slate-500">Loading Agent Garden…</p>
        </div>
      ) : error ? (
        ...
        <div className="flex flex-col items-center justify-center py-24">
          <p className="font-mono text-lg text-slate-400 dark:text-slate-500">
            amadeus@a2a:~$ <span className="cursor-blink text-slate-700 dark:text-slate-300">_</span>
          </p>
          <p className="text-xs font-mono text-slate-400 dark:text-slate-500 mt-3">No agents registered yet.</p>
          ...
          {filteredAgents.map((agent, index) => {
            ...
            return (
              <div
                ...
                className="stream-in bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-6 card-hover group relative flex flex-col hover:border-fuchsia-200 dark:border-fuchsia-500/30 hover:shadow-xl hover:shadow-fuchsia-500/5 transition-all duration-300"
              >
                ...
                <div className="absolute top-4 right-4 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-20">
                  <button onClick={() => toggleStar(agent.agent_id)} title="Favorite" className="p-1.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-400 dark:text-slate-500 hover:text-amber-500 hover:border-amber-200 dark:hover:border-amber-500/30 hover:bg-amber-50 dark:hover:bg-amber-900/30 rounded-lg shadow-sm transition-all">
                    <Star className={`w-3.5 h-3.5 ${starredAgents.includes(agent.agent_id) ? "fill-amber-400 text-amber-500" : ""}`} />
                  </button>
                  <button onClick={() => setSharingAgent({ id: agent.agent_id, name: agent.agent_name })} className="p-1.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-400 dark:text-slate-500 hover:text-fuchsia-600 hover:border-fuchsia-200 dark:border-fuchsia-500/30 hover:bg-fuchsia-50 dark:bg-fuchsia-900/30 rounded-lg shadow-sm transition-all">
                    <Share2 className="w-3.5 h-3.5" />
                  </button>
                  <button onClick={() => openEditModal(agent)} className="p-1.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 hover:border-slate-300 rounded-lg shadow-sm transition-all">
                    <Edit2 className="w-3.5 h-3.5" />
                  </button>
                  <button onClick={() => deleteAgent(agent.agent_id)} className="p-1.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-400 dark:text-slate-500 hover:text-red-600 hover:border-red-200 dark:border-red-500/30 hover:bg-red-50 dark:bg-red-900/20 rounded-lg shadow-sm transition-all">
                ...
                <div className="flex items-start gap-4 mb-4 pr-16">
                  {/* Styled Avatar */}
                  <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-fuchsia-500 to-cyan-600 flex items-center justify-center text-white shadow-md shadow-cyan-100 flex-shrink-0">
                    <Bot className="w-6 h-6" />
                  </div>
                  <div className="min-w-0">
                    <h3 className="section-head text-slate-900 dark:text-white text-base truncate leading-snug">{agent.agent_name}</h3>
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-[10px] font-mono text-slate-400 dark:text-slate-500">ID: {agent.agent_id?.substring(0, 8)}</span>
                    </div>
                  </div>
                </div>
                ...
                {/* Description */}
                {agent.description ? (
                  <p className="text-sm text-slate-600 dark:text-slate-400 mb-4 leading-relaxed line-clamp-2">{agent.description}</p>
                ) : (
                  <p className="text-sm text-slate-400 dark:text-slate-500 italic mb-4">No description configured.</p>
                )}

                {/* System Prompt Code Box */}
                <div className="mb-4 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 rounded-xl p-3 flex flex-col gap-1">
                  ...
                  <div className="text-xs font-mono text-slate-500 dark:text-slate-400 line-clamp-2 leading-relaxed">
                    {agent.agent_style || "The agent has no custom personality prompt configured."}
                  </div>
                </div>

                {/* Garden Metrics Row */}
                <div className="grid grid-cols-3 gap-2 py-3 border-t border-b border-slate-100 dark:border-slate-800 mb-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl px-2">
                  <div className="text-center">
                    <p className="text-[9px] font-mono text-slate-400 dark:text-slate-500">Invocations</p>
                    <p className="text-sm font-medium text-slate-800 dark:text-slate-200 mt-0.5">{metrics.runs} runs</p>
                  </div>
                  <div className="text-center border-x border-slate-100 dark:border-slate-800">
                    <p className="text-[9px] font-mono text-slate-400 dark:text-slate-500">Avg Latency</p>
                    <p className="text-sm font-medium text-slate-800 dark:text-slate-200 mt-0.5">{metrics.latency}s</p>
                  </div>
                  <div className="text-center">
                    <p className="text-[9px] font-mono text-slate-400 dark:text-slate-500">Success Rate</p>
                    <p className="text-sm font-medium text-slate-800 dark:text-slate-200 mt-0.5">{metrics.successRate}%</p>
                  </div>
                </div>
                ...
                <div className="mt-auto">
                  ...
                  {assignedTools.length > 0 ? (
                      ...
                      {assignedTools.map((tool: any) => {
                        ...
                        return (
                          <span
                            ...
                            className="badge badge-blue text-[9px] px-2 py-0.5 rounded-lg border border-cyan-100 flex items-center gap-1"
                          >
                    ...
                    <p className="text-xs text-slate-400 dark:text-slate-500 italic">No tools assigned to this agent.</p>
                  )}
              ...
              <form id="agent-form" onSubmit={handleSave} className="space-y-5">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input required value={formData.agent_name} onChange={e => setFormData({ ...formData, agent_name: e.target.value })} className="form-input rounded-xl border-slate-200 dark:border-slate-700" placeholder="e.g. Settlement Clerk Assistant" />
                  </div>
                  <div>
                ...
                <div>
                  ...
                  <input value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} className="form-input rounded-xl border-slate-200 dark:border-slate-700" placeholder="Primary task of the agent (e.g. extract document data and recommend transitions)..." />
                </div>
                <div>
                  <div className="flex items-center justify-between">
                    ...
                    <Select
                      value=""
                      onChange={(v) => {
                        const template = AGENT_TEMPLATES.find((t) => t.id === v);
                        if (template && template.agentStyle) {
                          setFormData({ ...formData, agent_style: template.agentStyle });
                        }
                      }}
                      options={AGENT_TEMPLATES.map((t) => ({ value: t.id, label: t.label }))}
                      placeholder="Load template…"
                      triggerClassName="rounded-lg text-[10px] py-1"
                    />
                  </div>
                  <textarea value={formData.agent_style} onChange={e => setFormData({ ...formData, agent_style: e.target.value })} className="form-input rounded-xl border-slate-200 dark:border-slate-700 h-28 resize-none font-mono text-xs" />
                </div>
                <div>
                  <label className="form-label">Assign MCP Tools <span className="normal-case font-normal text-slate-400 dark:text-slate-500">(select one or more)</span></label>
                  <MultiSelect
                ...
                <div>
                  <label className="form-label">Knowledge Bases <span className="normal-case font-normal text-slate-400 dark:text-slate-500">(select one or more)</span></label>
                  <MultiSelect
                    values={formData.knowledge_base_ids}
                    onChange={(vals) => setFormData({ ...formData, knowledge_base_ids: vals })}
                    options={knowledgeBasesList.map((kb) => ({
                      value: kb.kb_id,
                      label: kb.name,
                ...
                <div className="border-t border-slate-100 dark:border-slate-800 pt-5">
                  <div className="flex items-center justify-between mb-1">
                    <label className="form-label flex items-center gap-1.5">
                      <Workflow className="w-3.5 h-3.5 text-slate-400 dark:text-slate-500" /> Loop Mode
                    </label>
                    {modalMode === "edit" && formData.tools.length > 0 && (
                      <button
                        type="button"
                        onClick={() => setRecipeEnabled((v) => !v)}
                        className={`relative w-10 h-5 rounded-full transition-colors ${recipeEnabled ? "bg-cyan-600" : "bg-slate-200"}`}
                      >
                        <span className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white dark:bg-slate-900 rounded-full shadow transition-transform ${recipeEnabled ? "translate-x-5" : ""}`} />
                      </button>
                    )}
                  </div>
                  ...
                  {modalMode === "create" ? (
                    ...
                    <div className="space-y-4 mt-3">
                      {recipeError && (
                        <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-500/30 text-red-700 p-3 rounded-xl text-xs flex items-start gap-2">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <input value={recipe.label} onChange={(e) => setRecipe({ ...recipe, label: e.target.value })} className="form-input rounded-xl border-slate-200 dark:border-slate-700" placeholder="e.g. Danantara Survey Loop" />
                        </div>
                        <div>
                          <label className="form-label">Iterations</label>
                          <input type="number" min={1} max={20} value={recipe.iterations} onChange={(e) => setRecipe({ ...recipe, iterations: e.target.value })} className="form-input rounded-xl border-slate-200 dark:border-slate-700" />
                        </div>
                      </div>

                      <div className="space-y-3">
                        {recipe.steps.map((step, idx) => (
                          <div key={step.id} className="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl p-4 space-y-3">
                            <div className="flex items-center justify-between">
                              <span className="text-xs font-semibold text-slate-600 dark:text-slate-400">Step {idx + 1}</span>
                              <div className="flex items-center gap-1">
                                <button type="button" onClick={() => moveStep(idx, -1)} disabled={idx === 0} className="p-1 text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 disabled:opacity-30 disabled:cursor-not-allowed"><ArrowUp className="w-3.5 h-3.5" /></button>
                                <button type="button" onClick={() => moveStep(idx, 1)} disabled={idx === recipe.steps.length - 1} className="p-1 text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 disabled:opacity-30 disabled:cursor-not-allowed"><ArrowDown className="w-3.5 h-3.5" /></button>
                                <button type="button" onClick={() => removeStep(idx)} className="p-1 text-slate-400 dark:text-slate-500 hover:text-red-600"><Trash2 className="w-3.5 h-3.5" /></button>
                              </div>
                            </div>

                            <div className="grid grid-cols-2 gap-2">
                              <input value={step.label} onChange={(e) => updateStep(idx, { label: e.target.value })} placeholder="Label (e.g. Get disposable email)" className="form-input rounded-lg border-slate-200 dark:border-slate-700 text-xs" />
                              <input value={step.toolName} onChange={(e) => updateStep(idx, { toolName: e.target.value })} placeholder="MCP tool name (e.g. trigger_uipath_job)" className="form-input rounded-lg border-slate-200 dark:border-slate-700 text-xs font-mono" />
                            </div>
                            <p className="text-[10px] text-slate-400 dark:text-slate-500 -mt-1.5">Must match an MCP method exposed by one of this agent's attached tools — validated when the recipe runs.</p>

                            <div>
                              <label className="text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Args template (JSON)</label>
                              <textarea value={step.argsTemplateText} onChange={(e) => updateStep(idx, { argsTemplateText: e.target.value })} className="form-input rounded-lg border-slate-200 dark:border-slate-700 h-16 resize-none font-mono text-xs" />
                            </div>

                            <div className="grid grid-cols-2 gap-2">
                              <div>
                                <label className="text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Variant count (optional)</label>
                                <input type="number" min={1} value={step.variantCount} onChange={(e) => updateStep(idx, { variantCount: e.target.value })} className="form-input rounded-lg border-slate-200 dark:border-slate-700 text-xs" />
                              </div>
                              <div>
                                <label className="text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">On fault</label>
                                <Select
                                  value={step.onFault}
                            ...
                            {step.resolveEnabled && (
                              <div className="grid grid-cols-2 gap-2 pl-5">
                                <input value={step.resolverId} onChange={(e) => updateStep(idx, { resolverId: e.target.value })} placeholder="Resolver id" className="form-input rounded-lg border-slate-200 dark:border-slate-700 text-xs font-mono" />
                                <input value={step.nameTemplate} onChange={(e) => updateStep(idx, { nameTemplate: e.target.value })} placeholder="Name template (e.g. Get_Email_{{variant}})" className="form-input rounded-lg border-slate-200 dark:border-slate-700 text-xs font-mono" />
                              </div>
                            )}
                            ...
                            {step.pollForEnabled && (
                              <div className="space-y-2 pl-5">
                                <input value={step.pollForToolName} onChange={(e) => updateStep(idx, { pollForToolName: e.target.value })} placeholder="Status-check tool name" className="form-input rounded-lg border-slate-200 dark:border-slate-700 text-xs font-mono" />
                                <textarea value={step.pollForArgsTemplateText} onChange={(e) => updateStep(idx, { pollForArgsTemplateText: e.target.value })} className="form-input rounded-lg border-slate-200 dark:border-slate-700 h-14 resize-none font-mono text-xs" placeholder="Args template (JSON)" />
                                <div className="grid grid-cols-2 gap-2">
                                  <input value={step.terminalField} onChange={(e) => updateStep(idx, { terminalField: e.target.value })} placeholder="Terminal field (e.g. state)" className="form-input rounded-lg border-slate-200 dark:border-slate-700 text-xs" />
                                  <input value={step.terminalValuesText} onChange={(e) => updateStep(idx, { terminalValuesText: e.target.value })} placeholder="Terminal values (comma-separated)" className="form-input rounded-lg border-slate-200 dark:border-slate-700 text-xs" />
                                  <input value={step.successValuesText} onChange={(e) => updateStep(idx, { successValuesText: e.target.value })} placeholder="Success values (optional)" className="form-input rounded-lg border-slate-200 dark:border-slate-700 text-xs" />
                                  <div className="grid grid-cols-2 gap-2">
                                    <input type="number" value={step.timeoutSeconds} onChange={(e) => updateStep(idx, { timeoutSeconds: e.target.value })} placeholder="Timeout (s)" className="form-input rounded-lg border-slate-200 dark:border-slate-700 text-xs" />
                                    <input type="number" value={step.pollIntervalSeconds} onChange={(e) => updateStep(idx, { pollIntervalSeconds: e.target.value })} placeholder="Interval (s)" className="form-input rounded-lg border-slate-200 dark:border-slate-700 text-xs" />
                                  </div>
                                </div>
                              </div>
                            )}
                            ...
                            {step.verifyEnabled && (
                              <div className="space-y-2 pl-5">
                                <input value={step.verifyToolName} onChange={(e) => updateStep(idx, { verifyToolName: e.target.value })} placeholder="Verify tool name" className="form-input rounded-lg border-slate-200 dark:border-slate-700 text-xs font-mono" />
                                <textarea value={step.verifyArgsTemplateText} onChange={(e) => updateStep(idx, { verifyArgsTemplateText: e.target.value })} className="form-input rounded-lg border-slate-200 dark:border-slate-700 h-14 resize-none font-mono text-xs" placeholder="Args template (JSON)" />
                                <div className="flex items-center gap-2">
                                  <input value={step.checkField} onChange={(e) => updateStep(idx, { checkField: e.target.value })} placeholder="Check field" className="form-input rounded-lg border-slate-200 dark:border-slate-700 text-xs flex-1" />
                      ...
                      <details className="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl p-3">
                        <summary className="text-xs font-semibold text-slate-600 dark:text-slate-400 cursor-pointer">Resolvers (Advanced)</summary>
                        <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-1 mb-2">
                        ...
                        <textarea
                          ...
                          className="form-input rounded-lg border-slate-200 dark:border-slate-700 h-28 resize-none font-mono text-xs w-full"
                        />
                      </details>
                    </div>
                  )}
