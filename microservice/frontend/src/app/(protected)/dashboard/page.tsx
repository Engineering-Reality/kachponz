      ...
      {STEP_FLOW_DISPLAY.map((step, idx) => {
        ...
        return (
          <div key={step.key} className="flex items-center">
            <div
              className={`h-1.5 rounded-full transition-all ${ isCompleted ? "w-5 bg-emerald-400" : isCurrent ? "w-5 bg-blue-500 animate-pulse" : "w-5 bg-slate-200" }`}
...
export default function Dashboard() {
  ...
  const completeStep = async (id: string, step: string) => {
    try {
      const res = await fetch(`/api/transactions/${id}/steps/${step}/complete`, {
        method: "POST", headers: { "Content-Type": "application/json" },
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              ...
              <div className="m-4 p-3 text-xs bg-red-50 border border-red-200 rounded-lg text-red-700">
                ...
                <p className="text-red-400 mt-1">Verify the backend is running and reachable</p>
              </div>
                  ...
                  {transactions.map((tx) => {
                    ...
                    return (
                      <tr
                        key={tx.id}
                        onClick={() => selectTx(tx)}
                        className={`border-b border-slate-50 border-l-[3px] ${borderColor} cursor-pointer text-xs transition-colors ${ isSelected ? "bg-blue-50/70" : "hover:bg-slate-50" }`}
                      >
                        ...
                        <td className="py-2.5 px-3 text-slate-600 text-[10px] font-medium">{tx.type?.replace('_', ' ')}</td>
                        ...
                        <td className="py-2.5 px-3">
                          <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 text-[10px] font-medium rounded ${ tx.status === "completed" ? "bg-emerald-50 text-emerald-700" : tx.status === "failed" ? "bg-red-50 text-red-600" : "bg-blue-50 text-blue-600" }`}>
                            {tx.status}
                          </span>
                        </td>
              ...
              <div className="flex border-b border-slate-200 bg-white px-4 flex-shrink-0">
                {(["events", "a2a", "graph"] as const).map((t) => (
                  <button
                    key={t}
                    onClick={() => setTab(t)}
                    className={`px-3 py-2 text-[11px] font-medium border-b-2 transition-colors ${ tab === t ? "border-blue-500 text-blue-600" : "border-transparent text-slate-400 hover:text-slate-600" }`}
                  >
                      ...
                      <div className="space-y-1.5">
                        {txEvents.map((evt: any, idx: number) => (
                          <div key={evt.id || idx} className="flex items-start gap-3 bg-white border border-slate-100 rounded-lg p-3 text-xs">
                            <div className={`w-5 h-5 rounded-md flex items-center justify-center flex-shrink-0 mt-0.5 ${ evt.status === "completed" ? "bg-emerald-100 text-emerald-600" : evt.status === "failed" ? "bg-red-100 text-red-600" : "bg-blue-100 text-blue-600" }`}>
