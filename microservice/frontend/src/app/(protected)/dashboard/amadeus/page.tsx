        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="flex items-center gap-3">
              <h1 className="text-3xl font-semibold tracking-tighter vibrant-rainbow-text">
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <GlassCard className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="text-sm font-medium text-slate-900">E2E Settlement Demo</h3>
        ...
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
          <div className="px-5 py-3 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
            <h2 className="text-xs font-medium text-slate-600 flex items-center gap-1.5">
                ...
                return (
                  <div key={step.key} className="flex items-center">
                    ...
                    >
                      ...
                      <div className={`text-[10px] font-medium leading-tight mb-1.5 ${count > 0 ? "text-slate-800" : "text-slate-400"}`}>
                        {step.label}
                      </div>
                      <div className={`text-lg font-semibold tabular-nums ${count > 0 ? "text-blue-600" : "text-slate-300"}`}>
                        {count}
                      </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
          ...
          {loading ? (
            ...
            <div className="m-4 p-3 text-xs bg-red-50 border border-red-200 rounded-lg text-red-700">
              <div className="flex items-center gap-1.5 font-medium"><AlertTriangle className="w-3.5 h-3.5" /> {error}</div>
              <p className="text-red-400 mt-1">Ensure amadeus-core is running and reachable</p>
            </div>
          ) : displayed.length === 0 ? (
                  ...
                  {displayed.map((tx) => {
                    ...
                    return (
                      <tr key={tx.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors text-xs">
                        ...
                        <td className="py-3 px-4 text-slate-600 text-[10px] font-medium">
                          {tx.type?.replace(/_/g, " ")}
                        </td>
