...
export const registerToolsRoutes: FastifyPluginAsync = async (rootApp: FastifyInstance) => {
  ...
  app.get(
    ...
    async (request, reply) => {
      const res = await query(
        'SELECT * FROM tools WHERE company_id = $1 ORDER BY name ASC',
  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      const res = await query(
        'SELECT * FROM tools WHERE tool_id = $1 AND company_id = $2',
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      const res = await query(
        `INSERT INTO tools (name, description, versions, on_status, company_id)
  ...
  app.put(
    ...
    async (request, reply) => {
      ...
      values.push(id, request.auth!.companyId);

      const res = await query(
        `UPDATE tools SET ${fields.join(', ')} WHERE tool_id = $${idx++} AND company_id = $${idx} RETURNING *`,
        values
      );
  ...
  app.delete(
    ...
    async (request, reply) => {
      const { id } = request.params as { id: string };
      const res = await query(
        'DELETE FROM tools WHERE tool_id = $1 AND company_id = $2 RETURNING *',
        [id, request.auth!.companyId],
      );
