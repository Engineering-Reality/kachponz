 ...
 * (env.QWEN_VL_MODEL / Netra Runtime's on-prem "Qwen VLM", same
 ...
 * embedding, stays on-prem-capable when QWEN_MODE=on_prem.
 */
...
export async function extractTextFromImage(buffer: Buffer, mimeType: string): Promise<string> {
  ...
  const baseURL = runtime === 'on_prem'
    ...
    : env.QWEN_BASE_URL;
  const modelName = runtime === 'on_prem' ? 'Qwen VLM' : env.QWEN_VL_MODEL;
