...
export async function truncateAll(): Promise<void> {
  await query(
    'TRUNCATE a2a_task_messages, a2a_tasks, transaction_events, transactions, service_accounts, agents, tools, companies RESTART IDENTITY CASCADE',
  );
}

/** Insert a `companies` row — required FK target for agents/tools inserts. */
export async function seedCompany(id?: string): Promise<string> {
  const companyId = id ?? randomUUID();
  await query('INSERT INTO companies (company_id, name) VALUES ($1, $2)', [
    companyId,
    `test-co-${companyId.slice(0, 8)}`,
  ]);
  return companyId;
}
