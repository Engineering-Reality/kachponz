...
| 8 | **`trustProxy: true`** | `server.ts:30` | Low | Mempercayai seluruh rantai `X-Forwarded-For`. Bila rate-limit berbasis IP dipakai, `XFF` palsu bisa memintasnya. | Set `trustProxy` ke IP/subnet reverse-proxy on-prem yang diketahui. | OPEN |
...
## Prioritas tindakan
1. **#1 (RCE stdio)** dan **#2 (backend/.env)** — kerjakan lebih dulu.
2. **#3 (SSRF)**, **#4 (error leak)**, **#5 (rate-limit)** — berikutnya.
3. Sisanya (#6–#12) hardening bertahap.
